...
"""
function draw_points_with_GP_and_save(n_points::Int64; params_names::Array{String,1}, xdata::Array{Float64,2}, mean_f::Float64, ydata::Vector{Float64}, ydata_err::Vector{Float64}, kernel::Function, hparams::Vector{Float64}, prior_bounds::Union{Array{Tuple{Float64,Float64},1}, Nothing}=nothing, max_mean::Float64=Inf, max_std::Float64=Inf, max_post::Float64=Inf, save_path::String="")
    ...
    @time prior_draws_GP_table = predict_model_from_uniform_prior_until_accept_n_points(params_names, xdata, ydata, kernel, hparams, ydata_err, n_points; prior_bounds=prior_bounds, max_mean=max_mean, max_std=max_std, max_post=max_post)

    file_name = joinpath(save_path, "GP_train$(n_train)_meanf$(mean_f)_sigmaf$(hparams[1])_lscales$(round(sum(hparams[2:end]), digits=2))_vol$(round(vol, digits=2))_points$(n_points)_mean$(max_mean)_std$(max_std)_post$(max_post).csv")
    f = open(file_name, "w")
